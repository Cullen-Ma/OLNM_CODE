...
class CTViT(nn.Module):
    def __init__(
        self,
        *,
        dim,#Transformer在内部用来表示一个patch的向量长度
        codebook_size,#特征目录清单。每一行就是一条标准的详细信息(dim)。encoder后的feature在codebook里面找一个最相似的来替换
        image_size,
        patch_size,
        temporal_patch_size, 
        spatial_depth,
        ...
        # 新增/修改: 为PET数据定义一个独立的patch embedding模块，并深度拷贝以确保参数独立
        self.to_patch_emb_pet = copy.deepcopy(self.to_patch_emb)

        # 新增/修改: 定义融合层。输入维度是拼接后的 dim * 2, 输出维度是原始的 dim
    ...
    def forward(
        self,
        ct_video,         # 新增/修改: 将 'video' 参数重命名为 'ct_video'
        pet_video,        # 新增/修改: 增加 'pet_video' 参数
        mask = None,
        return_recons = False,
        ...
        assert ct_video.ndim in {4, 5}

        # 新增/修改: 增加对CT和PET数据维度一致性的断言
        assert ct_video.shape == pet_video.shape, "CT and PET data must have the same dimensions"

        is_image = ct_video.ndim == 4

        if is_image:
            ct_video = rearrange(ct_video, 'b c h w -> b c 1 h w')
            pet_video = rearrange(pet_video, 'b c h w -> b c 1 h w') # 新增/修改: 同样处理PET数据
            assert not exists(mask)

        b, c, f, *image_dims, device = *ct_video.shape, ct_video.device
        device=torch.device('cuda')
        assert tuple(image_dims) == self.image_size
        assert not exists(mask) or mask.shape[-1] == f

        # --- 拼接融合逻辑开始 ---
        # 新增/修改: 步骤1 - 分别将CT和PET数据转换为tokens
        ct_tokens = self.to_patch_emb(ct_video)
        pet_tokens = self.to_patch_emb_pet(pet_video)

        # 新增/修改: 步骤2 - 沿特征维度（最后一个维度）进行拼接
        concatenated_tokens = torch.cat((ct_tokens, pet_tokens), dim=-1)

        # 新增/修改: 步骤3 - 通过融合层进行信息融合与降维
        tokens = self.fusion_layer(concatenated_tokens)
        # --- 拼接融合逻辑结束 ---
        ...
        *_, h, w, _ = shape
        ...
        vq_mask = None
        if exists(mask):
            vq_mask = self.calculate_video_token_mask(ct_video, mask) # 使用ct_video计算mask
        ...
        # 重建损失以CT数据为基准
        if exists(mask):
            recon_loss = F.mse_loss(ct_video, recon_video, reduction = 'none')
            recon_loss = recon_loss[repeat(mask, 'b c t', c = c)]
            recon_loss = recon_loss.mean()
        else:
            recon_loss = F.mse_loss(ct_video, recon_video)
        ...
        if return_discr_loss:
            ...
            video = pick_video_frame(ct_video, frame_indices) # 使用ct_video进行loss计算
            recon_video_picked = pick_video_frame(recon_video, frame_indices)

            recon_video_picked = recon_video_picked.detach()
            video.requires_grad_()

            transform = T.Compose([T.Resize(256)])

            recon_video_picked = transform(recon_video_picked)
            video = transform(video)
            
            recon_video_discr_logits, video_discr_logits = map(self.discr, (recon_video_picked, video))
        ...
        # perceptual loss
        input_vgg_input = pick_video_frame(ct_video, frame_indices) # 使用ct_video进行loss计算
